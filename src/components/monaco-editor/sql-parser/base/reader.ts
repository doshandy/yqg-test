    ...
    switch (typePlusVariant) {
      ...
      case 'statement.select':
        ...
        if (
          ...
          const result = ssFromStatement.result;

          const aliasFields = result
            .filter((v) => {
              return v.alias?.value;
            })
            .map((v) => {
              const aliasName = _.get(v.alias, 'value');
              const realFieldName = _.get(v.name, 'name.value');
              const type = _.get(v.name, 'type');
              let realField;

              if (type === 'function') {
                const args = _.get(v.name, 'args', []);
                if (args && args.length > 0) {
                  const arg = args[0];
                  // 处理不同函数
                  if (realFieldName === 'substr') {
                    realField = {
                      label: _.get(arg, '1.name.name.value'),
                      insertText: _.get(arg, '1.name.name.value'),
                    };
                  }
                }
              }
            ...
            .map((field) => {
              const selectedField = ssFromStatement.result.find((result) => {
                ...
                if (
                  ...
                  return (
                    _.get(result.name, 'groupName.value') ===
              ...
              if (selectedField.alias) {
                return {
                  ...
                  insertText: selectedField.alias.value,
                  detail: 'alias',
                };
              }
          ...
          if (aliasFields.length > 0) {
            statementSelectFields = _.uniqBy(
              statementSelectFields.concat(aliasFields),
              'label',
            );
          }
        }
