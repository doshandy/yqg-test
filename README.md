...
一个 **完全单机、零后端依赖** 的数据平台演示工程，仿照内部项目 [`cn-data-pilot`](../cn-data-pilot) 与 [`cn-data-lumina`](../cn-data-lumina) 的 UI 与交互模式，可直接在浏览器里跑出完整的 **SQL 工作台 / 数据地图 / Copilot AI 对话** 体验。
...
```bash
pnpm install       # 首次安装依赖（约 300+ 包，耗时较长）
pnpm dev       # 启动 Vite，内嵌 MSW 拦截
```
...
```text
[MSW] Mocking enabled.
[MSW] Worker registered successfully.
```
...
pnpm typecheck    # vue-tsc --noEmit 独立的类型检查
pnpm preview      # 本地预览 dist/，依然带 MSW
```

> 类型检查从 build 流程里拆了出来：`build` 只跑 Vite（esbuild 转译），`typecheck` 用 `vue-tsc --noEmit` 独立校验。这样生产构建不会因为类型错阻塞，同时开发期/CI 仍能跑类型检查。
...
│   │       ├── data-map-agent.ts      # /data-map-agent（对话式数据探索，独立复刻）
│   │       ├── data-develop.ts        # /data-develop/task /data-develop/table /studio /ops /explore/sql
│   │       └── tag.ts                 # /tag
│   │
│   ├── pages/
│   │   ├── home/                      # Pilot/Lumina 两套首页 + 404
│   │   ├── workbench/                 # 工作台概览
│   │   ├── metric/catalog/            # 指标目录
│   │   ├── data-map/                  # 数据地图（资产列表 + 详情抽屉）
│   │   ├── data-map-agent/            # ⭐ 数据地图 Agent —— 完整对话式 UI（复刻自 cn-data-pilot）
...
│   │   ├── handlers.ts                # 所有接口 handler（≈ 100 个）
│   │   └── data/                      # 各业务模块的 mock 数据与剧本
│   │       ├── common.ts copilot.ts data-develop.ts datamap.ts explore.ts
│   │       ├── home.ts metric.ts ops.ts sql-sider.ts studio.ts tag.ts
│   │       └── data-map-agent.ts
│   │
│   └── utils/
│       ├── request.ts                 # 轻量 fetch 封装（响应结构对齐 @yqg/resource）
│       ├── monaco-setup.ts
│       └── tool.ts
│
├── vite.config.ts
...
|---|---|---|
| `/` | → `/pilot-home` | |
| `/pilot-home` | DataPilot 首页 | Canvas 粒子 + 打字机 + 鼠标跟随光效 |
| `/lumina-home` | DataLumina 首页 | 动效 SVG Logo + 霓虹光球 |
...
| **`/data-map-agent`** | **数据地图 Agent** ⭐ | 对话式数据探索：欢迎页 + 推荐提示词 + 流式思考链 + 会话历史 + 收藏表 + 分享（完整复刻 cn-data-pilot /data-map 体验）|
...
Copilot 在本项目里有 **两处挂载**，共享一套 SSE mock：
...
### 接口响应统一结构
所有 mock 返回保持和原项目 `@yqg/resource` 一致：
```ts
...
### 跨模块独立
`/data-map-agent` 是 cn-data-pilot 原版 data-map 的完整复刻（18 个文件、约 4300 行），**不复用** `components/copilot/` 下的 Studio Copilot 组件，自带 copilot-main / footer / sidebar / done-card / ai-message-feedback 等整套视图。这是为了方便未来跟原项目对齐同步。
...
- Jupyter Notebook（依赖内部 JupyterHub）
- G2 图表（依赖深度定制的 `@antv/g2`）
- 神策埋点 / Sentry / FMP 监控
- `@yqg/permission` 权限点与多国部署
- 真实认证 / 真实后端
- CI/CD 流水线、发布中心
...
**Q: 刷新后 MSW 报 404？**
A: 检查 `public/mockServiceWorker.js` 是否存在。如果没了就跑 `npx msw init public/ --save` 重新生成。
